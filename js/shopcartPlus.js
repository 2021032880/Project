...
$(function(){
    ...
    var $bodyInputs = $('tbody input[type=checkbox]');   //中间选择框
    ...
    $theadInput.change(function(){
        ...
        $totalPriceInput.prop('checked',checkState); //把状态给计算机总价中的全选框

        allTotal();//总计
    });
    /* 计算总价全选 
        1.给计算总价的全选按钮，绑定点击事件，获取绑定状态（ture/false）
        2.把状态给表头的全选
        3.把状态给表格中的选择框
    */
        $totalPriceInput.change(function(){
            ...
            $bodyInputs.prop('checked',checkState); //赋值给表头全选框
            $theadInput.prop('checked',checkState); //赋值给表格中全选框
            allTotal();//总计
        });
         ...
         * 
         */
        $bodyInputs.change(function(){ //给表格中单选框绑事件
            var flag = true; //定标杆为ruer
            ...
            $theadInput.prop('checked',flag); //把获取赋值给头部全选框
         ...
         */
             ...
             /*
                ...
                4.把减少后的值，重新赋值给前面的输入值
             */

            $('.reduce').click(function(){//绑定点击事件
                var count= parseInt($(this).prev().val());//获取前面输入框的值
                count--;//自减
                count=count < 1 ? 1 : count;//边界判断
                $(this).prev().val(count);//把减少后的值，重新赋值给前面的输入值
                 //小计
                 subTotal($(this),count);
                 allTotal();//总计
            })
            ...
            function subTotal(dom,count){
                ...
                dom.closest('tr').find('.subprice').text(subTotalPrice.toFixed(2));//把小计的额结果渲染到页面，保留两位小数
            }
            /*
